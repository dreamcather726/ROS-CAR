...
static constexpr int8_t WHEEL_ENC_L_SIGN = LEFT_MOTOR_INVERT_DIR ? -1 : 1;// 左轮方向修正：1=不变，-1=取反
static constexpr int8_t WHEEL_ENC_R_SIGN = RIGHT_MOTOR_INVERT_DIR ? -1 : 1;// 右轮方向修正：1=不变，-1=取反

static constexpr float WHEEL_DIAMETER_CM = 6.5f;// 轮子直径（单位：厘米）
static constexpr float WHEEL_COUNTS_PER_REV = 1750.0f; // 每转一次的计数
